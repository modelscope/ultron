...
class _MemoryMixin:
    """DB operations for memory records and memory search."""
    ...
    # ============ Segment memory invalidation ============

    def archive_memories_by_tag(self, tag: str) -> int:
        """Archive all active memories whose JSON tags array contains *tag*.

        Used when a task segment's fingerprint changes and its old memories
        should be retired.  Returns the number of rows updated.
        """
        if not tag:
            return 0
        # SQLite JSON: tags is stored as a JSON array string.  Use LIKE for
        # simple substring match — tag values are short, unique-ish prefixes.
        pattern = f'%"{tag}"%'
        with self._get_connection() as conn:
            cur = conn.cursor()
            cur.execute(
