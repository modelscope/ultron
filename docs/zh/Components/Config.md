...
Ultron 使用 `UltronConfig` 数据类进行配置管理。**环境变量 `ULTRON_*`** 与 `**DASHSCOPE_API_KEY**` 决定各字段的默认值；在代码里构造 `UltronConfig(...)` 时传入的参数 **优先于** 环境变量。`DASHSCOPE_API_KEY` 对应配置字段 `dashscope_api_key`；构造 `Ultron` 时若非空会写回 `os.environ["DASHSCOPE_API_KEY"]`，供现有 DashScope 客户端使用。

首次导入 `ultron.config`（或 `import ultron`）时，会调用 `load_ultron_dotenv()`：从 `~/.ultron/.env` 读取键值并写入 `os.environ`（`override=False`）。

- 仓库内提供 `**/.env.example`**，可将 `**DASHSCOPE_API_KEY`** 与各 `ULTRON_*` 一并写入 `**~/.ultron/.env**`（若目录不存在需先创建）。
...
| 配置项                            | 类型    | 默认值                                     | 说明                                      |
| ------------------------------ | ----- | --------------------------------------- | --------------------------------------- |
| `llm_model`                    | str   | `qwen3.5-flash`                          | 主链路 LLM（智能摄取、摘要与合并等）                    |
...
| `llm_api_url`                  | str   | `https://dashscope.aliyuncs.com/api/v1` | DashScope 兼容 API 根 URL                  |
